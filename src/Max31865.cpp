...
esp_err_t Max31865::clearFault() {
    ...
    // Write a 1 to this bit while writing 0 to bits D5, D3, and D2 to return all fault status bits
    // (D[7:2]) in the Fault Status Register to 0
    uint8_t resetByte = configByte;
    resetByte &= ~0x2Cu;
    resetByte |= (1u << MAX31865_CONFIG_FAULTSTATUS_BIT);
    err = writeSPI(MAX31865_CONFIG_REG, &resetByte, 1);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error resetting fault status: %s", esp_err_to_name(err));
        return err;
    }

    // Now put back orig config, with clear fault status still
    return writeSPI(MAX31865_CONFIG_REG, &configByte, 1);
}
...
esp_err_t Max31865::getRTD(uint16_t *rtd, Max31865Error *fault) {
    ...
    // Only do fault checking if we need to, as per optional argument
    if (fault != nullptr) {
        // Reset fault, important, otherwise prior errors will not be cleared from
        // client code holding onto a persistent reference or pointer.
        *fault = Max31865Error::NoError;
